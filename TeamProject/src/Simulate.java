...
public class Simulate {
	...
	public static void main(String args[]){
		...
		if(args.length!=4){
			throw new IllegalArgumentException("Usage: java Simulate <dLower> <dUpper> <iterations> <seed>");
		}else{
		...
		for(int i=dLower;i<dUpper;i++){
			for(int j=0;j<iterations;j++){
				...
				int tempHops = s.noOfHops(i,srcCyclic,srcCubical,destCyclic,destCubical);
				System.out.println("Dimension: "+i+". Source: ("+srcCyclic+","+srcCubical+"). Destination: ("+destCyclic+","+destCubical+"). Hops: "+tempHops);
				hops+=tempHops;
				
			}
			hops=hops/iterations;
			noOfHops.add(hops);
			dimensions.add(i);
			System.out.println("Dimension: "+i+". Avg hops: "+hops);
		}
		
		new Plot().xAxisTitle("Number of hops").yAxisTitle("Dimensions").xySeries(new AggregateXYSeries(noOfHops,dimensions));
	} 
}
